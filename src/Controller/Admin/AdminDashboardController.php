...
class AdminDashboardController extends AbstractController
{
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    //  PIPELINE KANBAN (HR: own only; ADMIN: all)
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    #[Route('/pipeline', name: 'admin_pipeline')]
    public function pipeline(EntityManagerInterface $em): Response
    {
        $user = $this->getUser();
        if ($this->isAdmin()) {
            $applications = $em->getRepository(Application::class)->findBy([], ['applicationDate' => 'DESC']);
        } else {
            $applications = $em->getRepository(Application::class)->createQueryBuilder('a')
                ->join('a.offer', 'o')
                ->where('o.recruiterId = :uid')
        ...
        return $this->render('back/pipeline/index.html.twig', [
            'pipeline' => $pipeline
        ]);
    }

    #[Route('/pipeline/update-status', name: 'admin_pipeline_update_status', methods: ['POST'])]
    public function pipelineUpdateStatus(Request $request, EntityManagerInterface $em): Response
    {
        ...
        if (!$appId || !$newStatus) {
            return $this->json(['success' => false, 'message' => 'Invalid data payload'], 400);
        }

        $application = $em->getRepository(Application::class)->find($appId);
        if (!$application) {
        ...
        if (in_array($newStatus, ['Nouvelle', 'Acceptée', 'Refusée', 'Entretien', 'En attente'])) {
            ...
            if ($candidate) {
                ...
                $ns->notify(
                    ...
                    'OFFER_DECISION',
                    $statusLabel,
                    'Votre candidature pour "' . $offer->getTitle() . '" a changé de statut: ' . $newStatus,
        ...
        return $this->json(['success' => false, 'message' => 'Invalid status']);
    }
